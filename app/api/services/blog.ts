...
export const list = async (page: number = 1, limit: number = 10) => {
  try {
    ...
    const { data, count, error } = await supabase
      .from("blogs")
      .select('id, title, descp, image, image_alt, image_caption, created_at, status, category_id, categories(id, name), author(id, name, designation, description, profile), tags, canonical_url, read_time, featured_blogs, meta_keywords', { count: "exact" })
